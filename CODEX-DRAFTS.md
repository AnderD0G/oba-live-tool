...
已验证：TypeScript 检查、九项自动测试（包含同进程多次请求、流式内容、会话/模型隔离、取消后复用、进程退出恢复和关闭清理）、生产构建。使用 Codex CLI 0.153.4 的真实合成评论测试：进程启动约 0.35 秒，三条回复首字 5.4/5.3/6.2 秒、完成 6.8/5.8/6.5 秒，均为同一个 PID。这不是延迟保证，也不能把旧版约 11 秒全部归因于进程启动。平台发送未测试（本功能没有发送操作）。
